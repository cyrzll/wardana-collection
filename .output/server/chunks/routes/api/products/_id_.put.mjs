...
const _id__put = defineEventHandler(async (event) => {
  ...
  let name = "", description = "", category_id = null, gender_id = null;
  ...
  for (let i = 0; i < variantsConfig.length; i++) {
    ...
    for (const file of files) {
      const filename = `${Date.now()}-${Math.round(Math.random() * 1e3)}-${file.filename}`;
  ...
  const firstVariant = processedVariants[0] || { price: 0, costPrice: 0, discount: 0};
  ...
  await prisma.product.update({
    ...
    data: {
      name,
      description,
      category: category_id ? { connect: { id: category_id } } : void 0,
      gender: gender_id ? { connect: { id: gender_id } } : void 0,
