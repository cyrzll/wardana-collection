...
const { data: transactions, refresh: refreshData } = await useFetch('/api/admin/cashflow/transactions')
...
const filteredTransactions = computed(() => {
  ...
  return transactions.value.filter(tx => {
    ...
    const matchesType = typeFilter.value === 'Semua Tipe' || tx.type === typeFilter.value
    
    return matchesSearch && matchesType
  }).sort((a, b) => {
    if (sortOption.value === 'Terbaru') return new Date(b.date) - new Date(a.date)
    if (sortOption.value === 'Terlama') return new Date(a.date) - new Date(b.date)
    if (sortOption.value === 'Nominal Tertinggi') return (b.amount || 0) - (a.amount || 0)
    return 0
  })
})
  ...
  <div class="p-8 space-y-8">
    <div class="flex justify-between items-end">
      ...
      <div class="flex gap-4">
        ...
        <button @click="refreshData" class="text-[10px] uppercase tracking-widest font-bold bg-soft-black text-white px-6 py-3 hover:bg-neutral-800 transition-all">
          Refresh
        </button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div class="relative">
        ...
        <select v-model="typeFilter" class="w-full pl-12 pr-4 py-4 border border-border text-[10px] uppercase tracking-widest font-bold appearance-none bg-white">
      ...
      <div class="relative">
        <select v-model="sortOption" class="w-full px-4 py-4 border border-border text-[10px] uppercase tracking-widest font-bold appearance-none bg-white">
    ...
    <div class="space-y-12">
      <div v-for="(group, month) in groupedTransactions" :key="month" class="space-y-4">
        <h3 class="text-xs uppercase tracking-[0.4em] font-bold text-muted border-b border-border pb-4 flex items-center gap-4">
          <span class="bg-neutral-100 px-3 py-1 text-soft-black">{{ month }}</span>
          <span class="h-px bg-border flex-grow"></span>
          <span class="text-[9px] text-muted">{{ group.length }} Transaksi</span>
        </h3>
        
        <div class="bg-white border border-border shadow-sm overflow-hidden">
          <table class="w-full text-left">
            <thead>
              <tr class="bg-neutral-50/50 border-b border-border">
            ...
            <tbody class="divide-y divide-border">
              <tr v-for="tx in group" :key="tx.id" class="hover:bg-neutral-50/50 transition-all group">
                <td class="p-6">
      ...
      <div v-if="Object.keys(groupedTransactions).length === 0" class="p-24 text-center border border-dashed border-border bg-neutral-50/50">
        <p class="text-xs text-muted italic uppercase tracking-widest">
          Tidak ada data transaksi yang ditemukan.
        </p>
      </div>
      
      <!-- Pagination Placeholder -->
      <div class="p-6 border border-border flex justify-between items-center bg-white shadow-sm">
        <p class="text-[10px] uppercase tracking-widest text-muted">Menampilkan {{ filteredTransactions.length }} transaksi</p>
